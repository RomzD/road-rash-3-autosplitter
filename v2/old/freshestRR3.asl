...
split{ 
	    ...
	    print("IN MAIN Line 58, current time is" + current.time.ToString("X") + " current sec is " + current.sec.ToString("X") + ("current time in hex(raw)") + current.time + " timeSubsctaction is " + current.timeSubsctaction + " time is" + current.time + " sec is " + current.sec);
		...
		return true;
				...
				print (" IN ELSE mainTimer is " + current.mainTimer + " mainTimer1 is " + current.mainTimer1 + " mainTimer3 is"+ current.mainTimer3 + " mainTimer4 is " + current.mainTimer4 
				+" timeSubsctaction is " + current.timeSubsctaction + " time is" + current.time + " sec is " + current.sec +" current.ingameSec " + current.ingameSec + " ingameMin " + current.ingameMin +
				" mainTimer.ToString(x) is " + current.mainTimer + " mainTimer1.toString(x) " +  current.mainTimer1.ToString("X") );
...
//print( "current.ingMin is " + current.ingameMin.ToString("X") + " current.ingameSec " + current.ingameSec.ToString("X") + " current.mainTimer " + current.mainTimer.ToString("X") + " current.mainTimer1 " + current.mainTimer1.ToString("X")
 //+ "current.Time is "+ current.time.ToString("X") + " current.sec is  " + current.sec.ToString("X"));
print (" IN ELSE mainTimer is " + current.mainTimer + " mainTimer1 is " + current.mainTimer1 + " mainTimer3 is"+ current.mainTimer3 + " mainTimer4 is " + current.mainTimer4 
				+" timeSubsctaction is " + current.timeSubsctaction + " time is" + current.time + " sec is " + current.sec +" current.ingameSec " + current.ingameSec + " ingameMin " + current.ingameMin +
				" mainTimer.ToString(x) is " + current.mainTimer + " mainTimer1.ToString(x) " +  current.mainTimer1.ToString("X") );
				int ingSec = Convert.ToInt32(current.ingameSec.ToString("X"));
				...
				int shortStr = Convert.ToInt16( current.time.ToString("X") + current.sec.ToString("X"),16);
 				string finalTenth = ((Double.Parse(    shortStr.ToString()  ))/60).ToString();
				print("Final tenth is" + finalTenth);
				finalTenth = finalTenth.IndexOf(",") == -1 ? "0,0": "0,"+ finalTenth.Substring(finalTenth.IndexOf(",")+ 1 , 1 ).ToString() ;
				double finalTime =  (double) finalTimeWithoutTenth  + Double.Parse(finalTenth);
				string trackTime = finalTime.ToString();
				print("ingSec"+ ingSec +" ingMin " + ingMin  +" finalTenth " + finalTenth +" finalTime " + finalTime +"trackTime" + trackTime);
	*/			
}
